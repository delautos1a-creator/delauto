...
export default function AdminPartners() {
  ...
  const openEdit = (p: Partner) => {
    setEditing(p);
    reset({ name: p.name, logo: p.logo ?? "", description: p.description ?? "", website: p.website ?? "", category: p.category, is_active: p.is_active, sort_order: p.sort_order });
    setOpen(true);
  };

  const onSubmit = async (data: FormData) => {
    setSaving(true);
    const payload = { ...data, logo: data.logo || null, description: data.description || null, website: data.website || null };
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24" />)}</div>
      ) : partners.length === 0 ? (
          ...
          {partners.map((p) => (
            <Card key={p.id} className={`border-border ${!p.is_active ? "opacity-60" : ""}`}>
              <CardContent className="p-4 flex items-center gap-4">
                {p.logo ? (
                  <img src={p.logo} alt={p.name} className="w-12 h-12 object-contain rounded" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!p.is_active && <p className="text-xs text-muted-foreground">Neaktivan</p>}
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => openEdit(p)}>
                    <Edit className="w-3.5 h-3.5" />
                  </Button>
                  <Button size="sm" variant="outline" className="text-red-400" onClick={() => del(p.id)}>
